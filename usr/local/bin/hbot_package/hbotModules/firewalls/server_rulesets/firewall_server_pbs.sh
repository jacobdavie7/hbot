...
function firewall_server_pbs
{
    ...
    echo -e "\nALLOW services IN"
        ...
        echo " - icmp ping (IN)"
            iptables -A INPUT -p icmp --icmp-type 8 -s 10.0.30.40 -m conntrack --ctstate NEW -j ACCEPT -m comment --comment "ACCEPT new incoming ping from docker server (uptime kuma)"
